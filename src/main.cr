...
when .fetch("build", false)
  exit Faaso::Commands::Build.new.run(
    ans, ans["FOLDER"].as(Array(String)))
when .fetch("deploy", false)
  exit Faaso::Commands::Deploy.new.run(
    ans, ans["FUNKO"].as(String)) if ans["--local"]
  Faaso.rpc_call(ARGV)
when .fetch("export", false)
  exit Faaso::Commands::Export.new.run(
...
when .fetch("scale", false)
  exit Faaso::Commands::Scale.new.run(
    ans, ans["FUNKO"].as(String), ans["SCALE"]) if ans["--local"]
  Faaso.rpc_call(ARGV)
when .fetch("secret", false)
  exit Faaso::Commands::Secret.new.run(
    ans, ans["FUNKO"].as(String), ans["SECRET"].as(String))
when .fetch("status", false)
  exit Faaso::Commands::Status.new.run(
    ans, ans["FUNKO"].as(String)) if ans["--local"]
  Faaso.rpc_call(ARGV)
